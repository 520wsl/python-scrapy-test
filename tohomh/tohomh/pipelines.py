...
from tohomh.items import ContentItem
...
class TohomhPipeline(object):
    ...
    def process_item(self, item, spider):
        if isinstance(item, TohomhItem):
            ...
            sql = "insert into comics (name, author, comic_url, comic_status, category,synoptic) values (%s, %s, %s, %s, %s,%s)"
            try:
                ...
                if is_exist:
                    ...
                    cursor.execute(sql, (
                        item['name'], item['author'], item['comicUrl'], item['comicStatus'], item['category'],
                        item['synoptic']))
                    self.db.commit()
